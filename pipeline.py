"""
Pipeline: live video -> YOLO detect+track -> line crossing -> annotated frame + counts.
Supports recording to file (day-based storage).
"""
import cv2
import threading
import time
...
from camera_source import open_source, read_frame, release
from detector import DetectorTracker
from line_crossing import Line, LineCrossingCounter
import config
from storage_paths import get_day_folder, get_default_video_path
...
class Pipeline:
    """Runs capture + detection + tracking + line crossing; supports recording."""
    ...
    def __init__(
        ...
        self._on_frame_callback: Optional[Callable[[Any], None]] = None
        self._on_counts_callback: Optional[Callable[[dict], None]] = None
    ...
    def set_record_path(self, path: Optional[str]) -> None:
        """Set path to record annotated video; None to stop recording."""
    ...
    def _run_loop(self) -> None:
        while self._running and self._cap:
            ...
            for cls_id, cnt in sorted(counts.items()):
                ...
                cv2.putText(annotated, f"{name}: {cnt}", (10, y_offset),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                y_offset += 25

            # Start writer on first frame if recording
            if self._record_path and self._writer is None:
            ...
            if self._on_frame_callback:
                self._on_frame_callback(annotated)
            if self._on_counts_callback:
                self._on_counts_callback(counts)
            time.sleep(0.001)
